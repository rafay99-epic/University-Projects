...
for n = 1:nmax
    ...
    y1=y-0.5;
    if sqrt(x1+y1) <= 0.1 % this sqrt function will return the square root of each function this <= in metlab means that x+y is less than 0.1 this 0.1 will represent the ratio of slip. 
      plot(x1,y1,'b.'); % this b will represent the colur blue
      inside = inside + 1;  % this is increment so the loop can work and will continue to work until the lop is ended
    else
      plot(x1,y1,'r.'); % this r will represent the red color
    end      
    hold on;
end
hold off;
